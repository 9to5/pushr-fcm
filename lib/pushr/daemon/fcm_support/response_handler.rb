        ...
        def handle_single(result, registration_id)
          if result.key?('error')
            if result['error'] == 'NotRegistered' || result['error'] == 'InvalidRegistration'
              Pushr::FeedbackFcm.create(app: message.app, failed_at: Time.now, device: registration_id, follow_up: 'delete')
            end
            ...
            # fail Pushr::Daemon::DeliveryError.new(@response.code, nil, msg, 'FCM', false)
